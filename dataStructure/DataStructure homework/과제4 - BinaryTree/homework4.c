...
void preorderBST(BinTree* root) {
    if (root) {
        //printf("[%d -> (%d, %d)", root->data->key, root->left->data->key, root->right->data->key);
        printf("[%d ->(", root->data->key);
        if (root->left) {
...
void display_help(void) {
    printf("문자 입력: d=자료삭제, h=트리높이, i=자료입력, l=자료출력, n=노드개수, p=노드연결출력, s=탐색, v=트리구조출력, g = 중위 순회, q=종료 \n");
}
...
void display_tree_structure(BinTree* node) {  //보너스 포인트 사용
    printf("(bonus point) Display your tree structure. \n");
}

void save_file(void) { // 업데이트 파일 세이브함 , 과제는 무관
    ...
    while ((input = getchar()) != 'q') {
        ...
        case 'v':
            /*
               ...
               가급적 과제3에서 나온 영상 출력 부분을 잘 활용하여 트리 구조를 그리기
            */
